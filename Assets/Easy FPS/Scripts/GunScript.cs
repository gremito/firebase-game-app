...
public class GunScript : MonoBehaviour {
	...
	*/
	...
	void OnGUI()
	{
		...
		if(mouseLookScript && HUD_bullets)
	...
	void Awake()
	{
		mouseLookScript = GameObject.FindGameObjectWithTag("Player").GetComponent<MouseLookScript>();
		player = mouseLookScript.transform;
		mainCamera = mouseLookScript.myCamera;
		...
		rotationLastY = mouseLookScript.currentYRotation;
		rotationLastX= mouseLookScript.currentCameraXRotation;
	}
...
//		if(Input.GetAxis("Fire2") != 0 && !reloading && !meeleAttack)
		if(CrossPlatformInputManager.GetButton("Fire2") && !reloading && !meeleAttack)
		{
	...
	{
		mouseLookScript.mouseSensitvity_notAiming = mouseSensitvity_notAiming;
		mouseLookScript.mouseSensitvity_aiming = mouseSensitvity_aiming;
	}
...
//		if(player.GetComponent<Rigidbody>().velocity.magnitude > 1 && Input.GetAxis("Fire1") == 0)
		if(player.GetComponent<Rigidbody>().velocity.magnitude > 1 && 
		   CrossPlatformInputManager.GetAxis("Fire1") == 0)
		{//ifnot shooting
...
//		if (Input.GetAxis ("Vertical") > 0 && Input.GetAxisRaw ("Fire2") == 0 && meeleAttack == false && Input.GetAxisRaw ("Fire1") == 0) 
	...
	*/
	void RotationGun()
	{
		...
		rotationDeltaY = mouseLookScript.currentYRotation - rotationLastY;
		rotationDeltaX = mouseLookScript.currentCameraXRotation - rotationLastX;
		...
		gunWeightX = Mathf.SmoothDamp (gunWeightX, mouseLookScript.currentCameraXRotation, ref velocityGunRotate.x, rotationLagTime);
		gunWeightY = Mathf.SmoothDamp (gunWeightY, mouseLookScript.currentYRotation, ref velocityGunRotate.y, rotationLagTime);
	...
	public void RecoilMath()
	{
		...
		mouseLookScript.wantedCameraXRotation -= Mathf.Abs(currentRecoilYPos * gunPrecision);
		mouseLookScript.wantedYRotation -= (currentRecoilXPos * gunPrecision);		 
			...
			if(bulletsInTheGun > 0)
			{
				...
				if (bullet)
					...
					Instantiate (
						...
						bulletSpawnPlace.transform.rotation);
				else
					print ("Missing the bullet prefab");

				/** shotflash */
				holdFlash = Instantiate(
					muzzelFlash[randomNumberForMuzzelFlash], 
...
//			handsAnimator.SetBool("aiming", Input.GetButton("Fire2"));
			handsAnimator.SetBool("aiming", CrossPlatformInputManager.GetButton("Fire2"));
			handsAnimator.SetInteger("maxSpeed", pmS.maxSpeed);
//			if(Input.GetKeyDown(KeyCode.R) && pmS.maxSpeed < 5 && !reloading && !meeleAttack/* && !aiming*/)
			if(CrossPlatformInputManager.GetButton("Reload") && pmS.maxSpeed < 5 && !reloading && !meeleAttack/* && !aiming*/)
			{
				StartCoroutine("Reload_Animation");
			}
		}
