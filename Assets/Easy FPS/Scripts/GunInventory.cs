...
//			StartCoroutine("Spawn",currentGunCounter);
//		}
		if (CrossPlatformInputManager.GetButton("WeaponChange"))
		{
			if (currentGunCounter > 0)
			{
				switchWeaponCooldown = 0;
	...
	IEnumerator Spawn(int _redniBroj)
	{
		...
		if (currentGun)
		{
			if (currentGun.name.Contains("Gun"))
			{
				...
				GameObject resource = Resources.Load(gunsIHave[_redniBroj].ToString()) as GameObject;
				currentGun =
					Instantiate(resource, transform.position, /*gameObject.transform.rotation*/
